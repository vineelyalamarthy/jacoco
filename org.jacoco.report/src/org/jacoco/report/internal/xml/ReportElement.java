...
public class ReportElement extends XMLElement {
	 ...
	 *             in case of problems with the underlying output
	 */
	public ReportElement classElement(final String name) throws IOException {
		return namedElement("class", name);
	}
	 ...
	 * 
	 * @param method
	 *            method object to write out
	 * @return 'method' element
	 * @throws IOException
	 *             in case of problems with the underlying output
	 */
	public ReportElement method(final IMethodCoverage method)
			throws IOException {
		final ReportElement element = namedElement("method", method.getName());
		element.attr("desc", method.getDesc());
		final int line = method.getFirstLine();
		if (line != -1) {
